...
app.use(express.json({ limit: '50mb' })); // Aumentar para 50MB (anteriormente era 10MB)
app.use(express.urlencoded({ limit: '50mb', extended: true }));

// Configuração adicional para tratar respostas lentas
app.use((req, res, next) => {
  // Aumentar timeout para todas as requisições
...
app.use(cors({ 
  ...
  credentials: true,
  // Configurar CORS para permitir requisições maiores
...
app.use((req, res, next) => {
  // Log mais detalhado para visualizar problemas
...
app.use((req, res, next) => {
  // Aumentar o timeout da conexão HTTP para uploads de imagens e arquivos
  if (req.url.includes('/api/messages/send')) {
    req.setTimeout(600000); // 10 minutos para rotas de envio de mensagens (anteriormente 5 min)
  }
...
server.timeout = 600000; // 10 minutos (anteriormente 5 min)

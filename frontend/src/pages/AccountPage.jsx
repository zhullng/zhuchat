...
const AccountPage = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const endpoint = `/api/transfers/${modalAction}`;
      const payload = modalAction === 'transfer' 
        ? { senderId: authUser._id, receiverEmail, amount: Number(amount) } 
        : { userId: authUser._id, amount: Number(amount) };
    ...
    <div className="flex items-center justify-center min-h-screen bg-gray-100 p-4">
      <div className="w-full max-w-4xl bg-white rounded-xl shadow-lg p-8 space-y-6">
        ...
        <div className="bg-blue-500 p-6 rounded-lg text-white text-center">
          ...
          <p className="text-4xl font-semibold">€{authUser?.balance?.toFixed(2) || 'Carregando...'}</p>
        </div>
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
          {['deposit', 'transfer', 'withdraw'].map(action => (
            <button key={action} onClick={() => { setModalAction(action); setShowModal(true); }} className="btn bg-blue-500 text-white w-full">
              {action === 'deposit' ? 'Depositar' : action === 'transfer' ? 'Transferir' : 'Sacar'}
            </button>
          ))}
        </div>
        ...
        <div className="space-y-4 bg-gray-50 p-4 rounded-lg max-h-80 overflow-y-auto">
          {transfers.length === 0 ? <p className="text-center text-gray-500">Nenhuma transação encontrada</p> : 
            transfers.map(transfer => (
              <div key={transfer._id} className="flex justify-between p-2 border-b border-gray-300">
                <p className="font-medium text-gray-700">{transfer.senderId === authUser._id ? 'Você' : transfer.receiverEmail}</p>
                <p className={transfer.senderId === authUser._id ? 'text-red-500' : 'text-green-500'}>
                  {transfer.senderId === authUser._id ? '-' : '+'}€{transfer.amount.toFixed(2)}
                </p>
              </div>
            ))
          }

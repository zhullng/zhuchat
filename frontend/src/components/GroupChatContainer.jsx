...
const GroupChatContainer = ({ isMobile = false, onBack }) => {
  ...
  const downloadFileFromBase64 = (fileData) => {
    try {
      ...
      // Nome padrão se não houver nome de arquivo
      const fileName = fileData.name || "arquivo_baixado";
      ...
      if (!base64Data || typeof base64Data !== 'string') {
        toast.error("Formato de dados inválido", { id: toastId });
        return;
      }
      ...
      if (!base64Content) {
        toast.error("Conteúdo do arquivo vazio ou inválido", { id: toastId });
        return;
      }
      
      // Converter para blob com tratamento de erro
      try {
        ...
        console.error("Erro ao processar dados base64:", error);
        toast.error("Erro ao processar arquivo. Formato inválido.", { id: toastId });
      }
    } catch (error) {
          ...
          {groupMessages.map((message) => {
            ...
            if (message.fileData) {
              try {
                ...
                if (typeof message.fileData === 'object') {
                  fileData = message.fileData;
                } else {
                  fileData = JSON.parse(message.fileData);
                }
                
                // Validar a estrutura dos dados do arquivo
                if (!fileData.name || !fileData.type || !fileData.data) {
                  console.error("Dados de arquivo incompletos:", fileData);
                  fileData = null;
                }
                ...
                <div className="chat-bubble flex flex-col relative">
                  ...
                  {fileData && (
                    <div className="flex items-center gap-2 bg-base-200 p-2 rounded-md mb-2">
                      {fileData.type && typeof fileData.type === 'string' && fileData.type.startsWith('video/') ? (
                        ...
                        <>
                          ...
                          <div className="flex-1 min-w-0">
                            <p className="text-sm font-medium truncate">{fileData.name || 'Arquivo'}</p>
